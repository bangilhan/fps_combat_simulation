...
```
combat_simulation/
├── index.html                 # 메인 시뮬레이션 파일 (GitHub Pages 배포용)
├── sample_dataset_kill_tick_info.csv  # 입력 데이터
├── README.md                   # 프로젝트 문서 (이 파일)
├── vercel.json                 # Vercel 배포 설정
├── crumb_logo.png              # 프로젝트 로고
├── map_data.json               # 맵 데이터
...
#### GitHub Pages 배포 지원
- **목적**: 팀원들과 웹 기반으로 프로젝트 공유
- **구현 내용**:
  - GitHub Pages 배포 가이드 추가
  - Vercel 배포 옵션 제공
  - 로컬 실행 방법 문서화
  - CSV 파일 크기별 공유 방법 정리
- **배포 URL**: `https://bangilhan.github.io/fps_combat_simulation/`
- **상태**: ✅ 완료

### UI/UX 개선

#### Dashboard/Info 섹션 레이아웃 최적화
- **문제점**:
  - 컨텐츠 폭이 너무 넓어 가독성 저하
  - 상단 제목이 아래로 밀려남
  - 주요 기능 리스트 항목 간 불필요한 여백
- **해결 방법**:
  1. **컨텐츠 폭 조정**
     - `max-width: 900px` → `1400px` → `1000px` (최종)
     - 적절한 읽기 폭으로 최적화
  2. **상단 패딩 조정**
     - `padding: 40px` → `padding: 20px 40px 40px 40px`
     - 상단 여백을 줄여 제목 위치 개선
  3. **리스트 간격 최적화**
     - 리스트 항목 `margin-bottom: 10px` → `2px`
     - 마크다운 파서에서 리스트 항목 사이 불필요한 `<br>` 태그 제거
     - 리스트 렌더링 로직 개선 (`</li><br><li>` 패턴 제거)
- **상태**: ✅ 완료

#### 마크다운 파서 개선
- **개선 내용**:
  - 리스트 항목 사이의 줄바꿈 처리 로직 개선
  - `<ul>` 태그 앞뒤의 불필요한 `<br>` 제거
  - 리스트 내부와 외부의 줄바꿈 구분 처리
- **상태**: ✅ 완료
...
- **추가 내용**:
  - 팀원들과 공유하기 섹션 추가
  - GitHub Pages, Vercel, 로컬 실행 방법 상세 가이드
  - CSV 파일 크기별 대안 제시
  - 개발 이력 및 변경사항 섹션 추가
- **상태**: ✅ 완료
